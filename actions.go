...
func Upload(c *gin.Context) {
	...
	// Move image to queue location

	err = c.SaveUploadedFile(file, queuePath)
	if nil != err {
		respond(c, 500, gin.H{
			"error":   "server failure",
			"message": "Failed to save image to queue location.",
		})
		return
	}
	...
	if !imageMoved {
		respond(c, 500, gin.H{
			"error":   "server failure",
			"message": "Failed to move image into storage location.",
		})
		return
	}
